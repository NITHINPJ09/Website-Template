        ...
        stage('---Deployment---') { 
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'Server-Credentials', keyFileVariable: 'KEY', usernameVariable: 'USERNAME')]) {
                    ...
                    if [ -d "$DIR" ]; then cd "$DIR"; git pull origin master; sudo cp -r * /var/www/html/; else git clone https://github.com/NITHINPJ09/Website-Template.git; cd "$DIR"; sudo cp -r * /var/www/html/; fi
                    exit
                    EOF'''
                }
            }
